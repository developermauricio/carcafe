...
(function ($, MapSVG, window) {
    ...
    function FormElement(options, formBuilder) {
        ...
        if (this.type == "region") {
            ...
            this.width =
                _this.formBuilder.filtersHide && !_this.formBuilder.modal
                    ? null
                    : options.width || "100%";
        } else if (this.type == "distance") {
            // this.options = this.formBuilder.getMarkersList();
            this.label =
                this.label || (options.label === undefined ? "Search radius" : options.label);
        ...
        if (this.html) {
            ...
            this.editor = CodeMirror.fromTextArea(txt, {
                mode: { name: "handlebars", base: "text/html" },
                matchBrackets: true,
                lineNumbers: true,
            });
            if (_this.formBuilder.admin) this.editor.on("change", this.setTextareaValue);
        }
        ...
        if (this.formBuilder.editMode) {
            ...
            if (this.type == "image") {
                ...
                this.formBuilder.mediaUploader.on("select", function () {
                    if (_this.formBuilder.mediaUploaderFor !== _this) return false;
                    var attachments = _this.formBuilder.mediaUploader
                        .state()
                        .get("selection")
                        .toJSON();
                ...
                _this.views.result
                    ...
                    .mselect2({
                        ...
                        ajax: {
                            url:
                                ajaxurl +
                                "?action=mapsvg_search_posts&post_type=" +
                                _this.post_type,
                            dataType: "json",
                            delay: 250,
                    ...
                    .on("select2:select", function (e) {
                        ...
                        _this.views.result
                            .find(".mapsvg-post-url")
                            .text(_this.post.url)
                            .attr("href", _this.post.url);
                ...
                if (_this.formBuilder.mapsvg.isGeo()) {
                    var locations = new Bloodhound({
                        ...
                        remote: {
                            url:
                                ajaxurl +
                                "?action=mapsvg_geocoding&address=%QUERY%&language=" +
                                this.language,
                            wildcard: "%QUERY%",
                    ...
                    thContainer.on("typeahead:select", function (ev, item) {
                        ...
                        var locationData = {
                            ...
                            img: _this.formBuilder.mapsvg.getMarkerImage(
                                _this.formBuilder.getData()
                            ),
                        };

                        _this.location = new MapSVG.Location(
                            locationData,
                            _this.formBuilder.mapsvg
                        );
                        ...
                        if (
                            _this.formBuilder.mapsvg
                                .getData()
                                .options.source.indexOf("/geo-calibrated/usa.svg") !== -1
                        ) {
                            ...
                            _this.formBuilder.mapsvg
                                .getData()
                                .options.source.indexOf("/geo-calibrated/world.svg") !== -1
                        ) {
                            ...
                            if (
                                select.length !== 0 &&
                                _this.location.address.administrative_area_level_1
                            ) {
                                var regionObject = _this.formBuilder.mapsvg
                                    .getData()
                                    .regions.filter(function (region) {
                                        return (
                                            ...
                                            region.id ===
                                                _this.location.address.country_short +
                                                    "-" +
                                                    _this.location.address
            ...
            if (this.type === "select" || this.type === "radio") {
                ...
                if (
                    ...
                    this.name == locationField.markerField &&
                    ...
                    _this.views.result.on("change", "select", function (e) {
                        ...
                        if (src) {
                            if (_this.formBuilder.marker) {
                                var marker = _this.formBuilder.mapsvg.getMarker(
                                    _this.formBuilder.marker.id
                                );
                                marker.setImage(src);
                                _this.views.result
                                    .closest(".mapsvg-form-builder")
                                    .find(".mapsvg-marker-image-btn img")
                                    .attr("src", src);
                            }
                ...
                if (_this.formBuilder.mapsvg.isGeo()) {
                    var locations = new Bloodhound({
                        ...
                        remote: {
                            url:
                                ajaxurl +
                                "?action=mapsvg_geocoding&address=" +
                    ...
                    if (this.searchByZip) {
                        ...
                        this.views.result
                            .find(".mapsvg-distance-fields")
                            .removeClass("search-by-zip");
                    }
    ...
    FormElement.prototype.toggleMarkerSelector = function (jQueryObj, e) {
        ...
        var images = MapSVG.markerImages.map(function (image) {
            return (
                '<button class="btn btn-default mapsvg-marker-image-btn mapsvg-marker-image-btn-choose ' +
                (currentImage == image.url ? "active" : "") +
                '"><img src="' +
                image.url +
                '" /></button>'
            );
        ...
        _this.markerSelector.append(
            '<div class=\'mapsvg-marker-selector-help\'>Upload marker images on: <em><i class="fa fa-database"></i> Database > <i class="fa fa-pencil-square-o"></i> Edit fields > Location > Markers > Upload</em></div>'
        );
    ...
    FormElement.prototype.redrawImages = function () {
        ...
        this.images &&
            this.images.forEach(function (image) {
                var img = $('<img class="mapsvg-data-thumbnail" />')
                    .attr("src", image.thumbnail)
                    .data("image", image);
                var imgContainer = $('<div class="mapsvg-thumbnail-wrap"></div>').data(
                    "image",
                    image
                );
    ...
    FormElement.prototype.setEditorEventHandlers = function () {
        ...
        var imgSelector = $("#marker-file-uploader")
            .closest(".form-group")
        ...
        this.views.edit.on("click", ".mapsvg-filter-insert-options", function () {
            ...
            if (objType == "Object") {
                ...
                if (fieldName == "id") {
                    field = {
                        options: _this.formBuilder.mapsvg.getData().regions.map(function (r) {
                    ...
                    field = {
                        options: _this.formBuilder.mapsvg.getData().regions.map(function (r) {
                            return { label: r.title, value: r.title };
                        }),
                    };
                } else {
        ...
        this.views.edit.on("keyup change paste", 'input[name="name"]', function () {
            ...
            if (_this.type != "region") _this.name = this.value;
            _this.views.result
                .find("label")
    ...
    FormElement.prototype.mayBeAddStatusRow = function () {
        var _this = this;
        this.templates.editStatusRow =
            this.templates.editStatusRow || $($("#mapsvg-edit-status-row").html());
        ...
        if (
            ...
            row2.length &&
            !(
                row1.find("input:eq(0)").val().trim() ||
                row1.find("input:eq(1)").val().trim() ||
                row1.find("input:eq(2)").val().trim()
            ) &&
            !(
                row2.find("input:eq(0)").val().trim() ||
                row2.find("input:eq(1)").val().trim() ||
                row2.find("input:eq(2)").val().trim()
            )
        ) {
            row2.remove();
        }
    ...
    FormElement.prototype.mayBeAddDistanceRow = function () {
        var _this = this;
        this.templates.editDistanceRow =
            this.templates.editDistanceRow || $($("#mapsvg-edit-distance-row").html());
        // if there's something in the last status edit field, add +1 status row
    ...
    FormElement.prototype.getSchema = function () {
        ...
        if (this.type == "select") {
            ...
            if (data.multiselect) data.db_type = "text";
            ...
            if (data.type == "region") {
                ...
                data.options.forEach(function (option, index) {
                    if (data.options[index].value === "") {
                        ...
                        data.options[index].disabled = MapSVG.parseBoolean(
                            data.options[index].disabled
                        );
                ...
                data.options.forEach(function (option, index) {
                    if (data.options[index].value === "") {
                        ...
                        data.options[index].default = MapSVG.parseBoolean(
                            data.options[index].default
                        );
                        // data.optionsDict[option.value] = option;
                    }
                });
        ...
        if (this.type == "distance") {
            ...
            if (data.distanceControl === "none") {
                data.distanceDefault = data.options.filter(function (o) {
                    return o.default;
                })[0].value;
            }
        }
    ...
    var FormBuilder = function (options) {
        ...
        if (!MapSVG.templatesLoaded[this.template]) {
            $.get(
                MapSVG.urls.templates + _this.template + ".html?" + Math.random(),
                function (data) {
                    $(data).appendTo("body");
                    MapSVG.templatesLoaded[_this.template] = true;
                    Handlebars.registerPartial(
                        "dataMarkerPartial",
                        $("#mapsvg-data-tmpl-marker").html()
                    );
                    if (_this.editMode) {
                        Handlebars.registerPartial(
    ...
    FormBuilder.prototype.viewDidLoad = function () {
        _this.formControls.forEach(function (control) {
            if (control.html) {
                ...
                control.htmlEditor = CodeMirror.fromTextArea(txt, {
                    mode: { name: "handlebars", base: "text/html" },
                    matchBrackets: true,
                    lineNumbers: true,
                });
    ...
    FormBuilder.prototype.setEventHandlers = function () {
        ...
        $(window)
            ...
            .on("keydown.form.mapsvg", function (e) {
                if (MapSVG.formBuilder) {
                    if ((e.metaKey || e.ctrlKey) && e.keyCode == 13) MapSVG.formBuilder.save();
                    else if (e.keyCode == 27) MapSVG.formBuilder.close();
                }
            });
        ...
        if (this.editMode) {
            ...
            this.view.on("click", "#mapsvg-data-btn-save-schema", function (e) {
                ...
                var counts = {};
                _this.formControls.forEach(function (elem) {
                ...
                else $.growl.error({ title: "Errors", message: errors.join("<br />") });
            });
            setTimeout(function () {
    ...
    FormBuilder.prototype.redraw = function (formElement) {
            ...
            _this.schema.forEach(function (elem) {
                if (_this.admin && _this.admin.isMetabox && elem.type == "post") {
                    ...
                    if (_this.filtersMode) {
                        ...
                        elem.value = _this.data
                            ? _this.data[elem.name]
                            : elem.value !== undefined
                            ? elem.value
                            : null;
                    }
                    ...
                    if (elem.type == "location" && !_this.editMode) {
                        ...
                        _this.mapsvg.setMarkerEditHandler(function () {
                            ...
                            //     marker.setImage(img);
                            // },2000);
                            _this.locationFormElement &&
                                _this.locationFormElement.renderMarker(
                                    _this.mapsvg.getMarker(_this.marker.id)
                                );
                        });
                    ...
                    if (_this.filtersMode) {
                        if (
                            !_this.filtersHide ||
                            (_this.filtersHide && _this.modal && elem.type !== "search") ||
                            (!_this.modal && elem.type === "search")
                        ) {
        ...
        if (!_this.editMode) {
            if (this.schema.length == 0 && !_this.filtersMode) {
        ...
        if (_this.filtersMode && _this.filtersHide && !_this.modal) {
            this.showFiltersButton = _this.add({
                type: "modal",
                buttonText: _this.mapsvg.getData().options.filters.showButtonText,
            });
        }
    ...
    FormBuilder.prototype.add = function (params) {
        ...
        if (
            ...
            var repeat = false;
            _this.formControls.forEach(function (control) {
                if (control.type == params.type) repeat = true;
            });
            if (repeat) {
                $.growl.error({
                    title: "Error",
                    message: 'You can add only 1 "' + MapSVG.ucfirst(params.type) + '" field',
                });
        ...
        if (this.editMode) this.edit(formElement);
        return formElement;
    };

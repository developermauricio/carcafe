...
(function ($) {
    ...
    methods = {
        ...
        save: function (skipMessage) {
            ...
            var data = {
                mapsvg_data: MapSVG.convertToText(options),
                title: options.title,
                map_id: _data.options.map_id,
                region_prefix: options.regionPrefix,
            ...
            if (_this.mapsvgCssChanged) data.css = _this.mapsvgCss;
            if (options.manualRegions !== undefined) {
                data.manualRegions = MapSVG.parseBoolean(_this.manualRegions);
            }

            return $.post(
                ajaxurl,
                { action: "mapsvg_save", _wpnonce: _data.options._wpnonce, data: data },
                function (id) {
                    if ($.isNumeric(id)) {
        ...
        mapCopy: function (e) {
            ...
            if (!(new_name = prompt("Enter new map title", map_title + " - copy"))) return false;

            $.post(
                ajaxurl,
                { action: "mapsvg_copy", _wpnonce: nonce, id: id, new_name: new_name },
                function (new_id) {
        ...
        resizeSVGCanvas: function () {
            ...
            var s = Math.floor(msvg.getData().$leftSidebar.outerWidth());
            var s2 = Math.floor(msvg.getData().$rightSidebar.outerWidth());
            var h = msvg.getData().$header.is(":hidden")
                ? 0
                : Math.floor(msvg.getData().$header.outerHeight(true));
            var f = 0; //Math.floor(msvg.getData().$footer.outerHeight(true));
        ...
        setMode: function (mode, dontSwitchTab) {
            ...
            if (mode == "editRegions") {
                ...
                msvg.getData().$map.removeClass("mapsvg-edit-objects");
            } else if (mode == "draw") {
                msvg.setMarkersEditMode(false);
                ...
                _this.setDrawMode(false);
                msvg.getData().$map.removeClass("mapsvg-edit-regions");
                msvg.getData().$map.addClass("mapsvg-edit-objects");
                $("body").on("click.switchTab", ".mapsvg-region", function () {
                    $('#mapsvg-tabs-menu a[href="#tab_database"]').tab("show");
                });
        ...
        setEventHandlersMainScreen: function () {
            ...
            $("#mapsvg-admin").on("click", "#mapsvg-btn-activate", function (e) {
                ...
                $.post(
                    ...
                    {
                        purchase_code: code,
                        action: "mapsvg_save_purchase_code",
                        _wpnonce: MapSVG.nonce,
                    },
                    function (data) {
                        data = JSON.parse(data);
                        if (data.error_message) {
                            $.growl.error({ title: "", message: data.error_message });
                        } else if (data && data.ok) {
                            $("#mapsvg-alert-activate").hide();
                            alert(
                                'MapSVG is activated. You will receive updates notifications on the "Plugins" page and ' +
                                    "you will be able to update the plugin there."
                            );
                        }
                    }
                ).always(function () {
                    $("#mapsvg-btn-activate")._button("reset");
                });
            });
            ...
            $(".select-map-list")
                ...
                .on("select2:select", function () {
                    ...
                    if (link) window.location = link + "&noheader=true";
                });
            ...
            $("#svg_file_uploader").on("change", function (event) {
                $.each(event.target.files, function (index, file) {
                    ...
                    reader.onload = function (event) {
                        ...
                        var fileSizeInMb = object.data.length / (1024 * 1024);
                        ...
                        if (fileSizeInMb > postMaxSizeInMb) {
                            $.growl.error({
                                title: "Error",
                                message:
                                    "Can't upload the file. File size (" +
                                    fileSizeInMbString +
            ...
            $("#svg_file_uploader_form").on("submit", function (form) {
                ...
                files.forEach((file) => {
                    var blob = new Blob([file.data], { type: "text/xml" });
                    formData.append("file", blob, file.filename.replace(/\s/g, "_"));
                });
                formData.append("action", "mapsvg_upload");
                formData.append("_wpnonce", MapSVG.nonce);
                ...
                $.ajax({
                    ...
                    .done(function (filename, status, xhr) {
                        $.growl.notice({
                            title: "",
                            message: "File uploaded:<br>user-uploads/" + filename,
                        });
                        var o = $("#mapsvg-svg-file-select").find(
                            '[data-link="?page=mapsvg-config&map_id=new&package=uploads&path=' +
                                filename +
                                '"]'
                        );
                        if (!o.length)
                            $("#mapsvg-svg-file-select").append(
                                '<option data-link="?page=mapsvg-config&map_id=new&package=uploads&path=' +
                                    filename +
                                    '">user-uploads/' +
                                    filename +
                                    "</option>"
            ...
            $("#image_file_uploader_form").on("submit", function (form) {
                ...
                $.ajax({
                    ...
                    .done(function (data, status, xhr) {
                        var filenameFinal = data;
                        window.location.href =
                            "?page=mapsvg-config&map_id=new&package=uploads&path=" +
                            filenameFinal +
                            "&noheader=true";
                    })
                    .fail(function (response) {
            ...
            $("#save-api-key").on("click", function () {
                ...
                if (key && key.length) {
                    $.post(ajaxurl, {
                        action: "mapsvg_save_google_api_key",
                        maps_api_key: key,
                        geocoding_api_key: key2,
                        _wpnonce: MapSVG.nonce,
                    }).done(function (data) {
                        data = JSON.parse(data);
                        if (data.ok) {
        ...
        init: function (options) {
            ...
            $(document).ready(function () {
                ...
                if (onEditMapScreen) {
                    ...
                    if (_data.options.mapsvg_options.manualRegions) {
                    ...
                    _data.options.mapsvg_options.afterLoad = function () {
                        ...
                        new MapSVG.ResizeSensor($(".mapsvg-header")[0], function () {
                            setTimeout(function () {
                                _this.resizeSVGCanvas();
                            }, 1);
                        });
                        ...
                        if (!_data.options.mapsvg_options.defaultMarkerImage) {
                            msvg.setDefaultMarkerImage(_data.options.markerImages[0].url);
                        }
                        // MapSVG.defaultMarkerImage = (_data.options.markerImages && _data.options.markerImages.length)
                        ...
                        if (!hbData.title) {
                            ...
                            hbData.title =
                                hbData.title.charAt(0).toUpperCase() + hbData.title.substr(1);
                        }

                        msvg.update({ title: hbData.title });

                        if (
                            _data.options.mapsvg_options.extension &&
                            $().mapSvg.extensions &&
                            $().mapSvg.extensions[_data.options.mapsvg_options.extension]
                        ) {
                        ...
                        // _data.view.find('.mapsvg-select2').mselect2({
                    ...
                    if (
                        !_data.options.mapsvg_options.googleMaps.apiKey &&

...
class MapSVGMap {
    ...
    setSize(width, height, responsive) {
        ...
        if (_this.markers)
            _this.markersAdjustScreenPosition();
        if (_this.options.labelsRegions.on) {
            _this.labelsRegionsAdjustScreenPosition();
        }
    ...
    showUserLocation(callback) {
        ...
        this.getUserLocation(function (geoPoint) {
            ...
            _this.userLocationMarker.adjustScreenPosition();
    ...
    updateSize() {
        var _this = this;
        _this.scale = _this.getScale();
        _this.controllers.popover && _this.controllers.popover.adjustScreenPosition();
        _this.markersAdjustScreenPosition();
        if (_this.options.labelsRegions.on) {
            _this.labelsRegionsAdjustScreenPosition();
        }
    ...
    movingItemsAdjustScreenPosition(deltaX, deltaY) {
    ...
    markersMoveScreenPositionBy(deltaX, deltaY) {
        ...
        this.markersClusters.forEach(function (cluster) {
    ...
    scrollMove(e) {
        ...
        _this.movingItemsAdjustScreenPosition((_this.scroll.gx - ce.clientX), (_this.scroll.gy - ce.clientY));
    ...
    convertSVGToPixel(svgPoint) {
        ...
        if (_this.options.googleMaps.on) {
            if ((_this.viewBox.x - _this.svgDefault.viewBox.x) > _this.svgDefault.viewBox.width) {
                ...
                shiftXByGM = worldMapWidth * Math.floor((_this.viewBox.x - _this.svgDefault.viewBox.x) / _this.svgDefault.viewBox.width);
            }
        }
        let shiftXByScrollpane = _this.scroll.tx;
        let shiftYByScrollpane = _this.scroll.ty;
        let screenPoint = new ScreenPoint((svgPoint.x - _this.svgDefault.viewBox.x + shiftXByGM) * scale + shiftXByScrollpane, (svgPoint.y - _this.svgDefault.viewBox.y + shiftYByGM) * scale + shiftYByScrollpane);
        return screenPoint;
    }
        ...
        if (_this.options.labelsRegions.on) {
            _this.regions.forEach(function (region) {
                if (!region.textLabel) {
                    ...
                    $(_this.layers.labels).append(region.textLabel);
                }
                try {
        ...
        if (_this.options.labelsMarkers.on) {
            _this.markers.forEach(function (marker) {
                if (!marker.textLabel) {
                    ...
                    $(_this.layers.labels).append(marker.textLabel);
                }
                try {
                ...
                marker.adjustLabelScreenPosition();
            });
        }
    }
    addLayer(name) {
...
export { MapSVGMap as map, MapSVGMap as MapSVGMap, MapsRepository as mapsRepository, MapsV2Repository as mapsV2Repository, Repository as repository, SchemaRepository as schemaRepository, MapSVG as globals, Marker as marker, Location as location, Server as server, ArrayIndexed as arrayIndexed, Schema as schema, ScreenPoint as screenPoint, SVGPoint as svgPoint, GeoPoint as geoPoint };
